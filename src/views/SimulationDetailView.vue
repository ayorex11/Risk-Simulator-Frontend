...
const startPolling = () => {
  ...
  pollInterval.value = setInterval(async () => {
    ...
    try {
      ...
      const wasExecuting = executing.value
      const updated = await simulationStore.fetchSimulation(id, { silent: true })
      simulation.value = updated
      ...
      if (updated.status === 'completed' && updated.completed_at) {
        ...
        try {
          loadedResults =
            updated.result || (await simulationStore.fetchSimulationResults(id))
        } catch {
          loadedResults = null
        }

        // Only stop polling when real non-zero financial data is present.
        // This prevents stopping on a zeroed placeholder or a result object
        // that the DB transaction hasn't fully committed yet.
        const hasRealResults =
          loadedResults &&
          (
            Number(loadedResults.total_financial_impact) > 0 ||
            Number(loadedResults.risk_score) > 0
          )
        ...
        if (hasRealResults) {
          ...
          // Only toast when this tab actually kicked off or resumed a run (avoids stale toasts).
          if (wasExecuting) {
            showToast('✅ Simulation completed successfully!', 'success')
          }
        }
        // If results aren't populated yet, keep polling

      } else if (updated.status === 'failed') {
        stopPolling()
        executing.value = false
        showToast('❌ Simulation failed. You can retry execution.', 'error')
      }
    } catch (err) {
...
const financialChartOption = computed(() => {
  ...
  if (results.value.total_cascading_impact && Number(results.value.total_cascading_impact) > 0) {
    data.push({
      name: 'Cascading',
      value: Number(results.value.total_cascading_impact),
...
const loadSimulation = async () => {
  ...
  try {
    const id = route.params.id
    simulation.value = await simulationStore.fetchSimulation(id, { silent: true })

    if (simulation.value?.status === 'completed') {
      try {
        results.value = simulation.value.result
          || await simulationStore.fetchSimulationResults(id)
      } catch {
        results.value = simulation.value.result || null
      }
    } else if (simulation.value?.status === 'running') {
      // Page loaded while simulation already in progress — resume polling
      executing.value = true
      startPolling()
    }
  } catch (err) {
    error.value = err.response?.data?.error || 'Failed to load simulation'
    vToast.error(error.value)
  } finally {
    loading.value = false
  }
}
...
const executeSimulation = async () => {
  ...
  try {
    ...
    // Optimistically flip status so the UI shows running immediately
    if (simulation.value) simulation.value.status = 'running'
    startPolling()
  } catch {
    executing.value = false
...
const getProcessName = (proc) => {
  ...
  if (typeof proc === 'string') return `Process ${proc.slice(0, 8)}…`
  return 'Unknown Process'
}

const getProcessInitial = (proc) =>
  getProcessName(proc).charAt(0).toUpperCase()
